...
function parseJsDocTags(node: Node): {
	...
	for (const jsDoc of jsDocs) {
		...
		for (const tag of tags) {
			...
			const text = typeof tagText === 'string' ? tagText : tagText?.map((c: any) => c.getText()).join('') || '';
	...
	for (const [, declarations] of exportedDeclarations) {
		for (const declaration of declarations) {
			...
			try {
				...
				console.warn(`Failed to parse ${declaration.getKindName()} "${(declaration as any).getName?.() || 'unknown'}":`, error);
			}
		}
	}
