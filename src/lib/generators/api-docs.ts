...
function linkType(type: string, config: MarkdownGeneratorConfig): string {
	...
	const builtIns = ['string', 'number', 'boolean', 'void', 'any', 'unknown', 'never', 'null', 'undefined'];
 ...
 */
function generateSourceLink(source: { file: string; line: number }, config: MarkdownGeneratorConfig): string {
 ...
 */
function generateReturns(returnType: string, returnDescription: string | undefined, config: MarkdownGeneratorConfig): string {
	if (returnType === 'void') {
		return '';
	}
...
function generateFunctionDocs(func: ApiFunction, config: MarkdownGeneratorConfig): string {
	...
	// Signature
	const typeParams = func.typeParameters && func.typeParameters.length > 0
...
function generateClassDocs(cls: ApiClass, config: MarkdownGeneratorConfig): string {
	...
	// Signature
	const typeParams = cls.typeParameters && cls.typeParameters.length > 0
...
function generateInterfaceDocs(iface: ApiInterface, config: MarkdownGeneratorConfig): string {
	...
	// Signature
	const typeParams = iface.typeParameters && iface.typeParameters.length > 0
...
function generateTypeAliasDocs(type: ApiTypeAlias, config: MarkdownGeneratorConfig): string {
	...
	// Signature
	const typeParams = type.typeParameters && type.typeParameters.length > 0
